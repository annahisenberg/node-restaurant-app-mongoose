    ...
    it('should return restaurants with right fields', function() {
      ...
      return chai.request(app)
        ...
        .then(function(res) {
          ...
          resRestaurant = res.body.restaurants[0]; //restaurant from body
          return Restaurant.findById(resRestaurant.id); //restaurant from database
        })
        .then(function(restaurant) { //restaurant is in the database

          expect(resRestaurant.id).to.equal(restaurant.id); //making sure restaurant from body is same as restaurant form database
